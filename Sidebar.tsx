...
export default function Sidebar({ isAdminOrDev }: { isAdminOrDev: boolean }) {
  ...
  const [openMenu, setOpenMenu] = useState<'ranking' | 'backend' | null>(null);
  const railRef = useRef<HTMLDivElement>(null);

  // ปิดเมนูเมื่อคลิกนอกแถบ หรือเปลี่ยนหน้า
  useEffect(() => {
  ...
  return (
    <div ref={railRef} className="w-16 bg-[var(--rail)] h-screen flex flex-col items-center py-4.5 flex-shrink-0 relative z-30">
      ...
      <div className="relative">
        <button
          type="button"
          onClick={() => setOpenMenu((v) => (v === 'ranking' ? null : 'ranking'))}
          className="block"
        >
          <RailIcon icon="📊" label="Ranking" active={isActive('/ranking')} />
        </button>
        {openMenu === 'ranking' && (
          <div className="flex absolute left-16 top-0 h-auto w-60 bg-[var(--panel)] border-r border-[var(--line)] shadow-lg p-3.5 flex-col z-40 rounded-r-lg">
      ...
      {isAdminOrDev && (
        <div className="relative">
          <button
            type="button"
            onClick={() => setOpenMenu((v) => (v === 'backend' ? null : 'backend'))}
            className="block"
          >
            <RailIcon icon="🛠" label="หลังบ้าน" active={isActive('/backend') || isActive('/roster') || isActive('/users')} />
          </button>
          {openMenu === 'backend' && (
            <div className="flex absolute left-16 top-0 h-auto w-64 bg-[var(--panel)] border-r border-[var(--line)] shadow-lg p-3.5 flex-col z-40 rounded-r-lg">
              <div className="text-[11px] uppercase tracking-wide text-[var(--ink-soft)] mb-2 px-1.5">
                หลังบ้าน (Admin / Dev)
              </div>
